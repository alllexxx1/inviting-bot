...
@router.callback_query(
    StateFilter(FSMRegisterUser.subscribe_channels),
    F.data.in_(['subscribed'])
)
async def check_tg_channels_subscription(callback: CallbackQuery, bot: Bot, state: FSMContext):
    user_id = callback.from_user.id
    try:
        member = await bot.get_chat_member(CHANNEL_TO_CHECK, user_id)
        if member.status == 'member':
            await callback.message.delete()
            await callback.message.answer('Отлично. Теперь отправьте скриншот вашего поста, пожалуйста.')
            await state.set_state(FSMRegisterUser.send_valid_pass)
        else:
            await callback.message.answer('Вы не подписаны. Попробуйте еще раз. Если считаетк, что прозошла ошибка напишите нам.')
